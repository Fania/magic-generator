...
int numX2X4X6Combos, countMagic;

// Make Franklin squares and transform to most-perfect. 
...
int openWrite() {
//  ---------
  const int bufSize = 128; char buf[bufSize]; int sub = 0;
  const char *defaultName = "Order8Most-perfectSquares";
...
void swapRowsCols(Uint **x, int n, int i, int j)
//   ------------
{
  int nm1 = n-1, k = nm1-i, l = nm1-j; Uint *tmp;
  tmp = x[i]; x[i] = x[j]; x[j] = tmp;
  tmp = x[k]; x[k] = x[l]; x[l] = tmp;
  for (int r = 0; r < n; ++r) {
    Uint tmp;
    tmp = x[r][i]; x[r][i] = x[r][j]; x[r][j] = tmp;
    tmp = x[r][k]; x[r][k] = x[r][l]; x[r][l] = tmp;
  }
} // swapRowsCols
//-------------------------------------------------------------------------------

const int bytesPerSquare = 193, numMagic = 368640,
          sizeMagic = numMagic*bytesPerSquare;
char bufferMagic[sizeMagic], *sMagic = bufferMagic;
void printSquare(Uint **x) {
//   -----------
  swapRowsCols(x, N, 2, 4); // Franklin to most-perfect
  char *s = sMagic;
  for (int r = 0; r < N; ++r) {
    for (int c = 0; c < N; ++c) {
  ...
  *s++ = '\n'; sMagic = s;
  swapRowsCols(x, N, 2, 4); // reverse, (to Franklin)
} // printSquare
//--------------------------------------------------------------------------
...
//   ------------
  ...
  printVariations(xRows); countMagic += squareVariations;
} // printSquares
//--------------------------------------------------------------------------
...
void writeSquares(int wfdM) {
//   ------------
  if (_write(wfdM, bufferMagic, sizeMagic) != sizeMagic)
    perror("Error writing squares: ");
} // writeSquares
//--------------------------------------------------------------------------
          
int main() {
//  ----
  outputLocalTime(stdout);
  int wfdM = openWrite();
  if (wfdM != -1) {
    time_t startTime = time(NULL);
    getSquares(); writeSquares(wfdM);
    printf("\nSquares: %u\n", countMagic);
