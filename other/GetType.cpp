 ...
 * Updated: 2020-07-13
 *   Add isUltramagic, isCompact, isComplete, isMostPerfect for prime number magic squares
 * Updated: 2020-09-27
 *   Fix open_input when file name includes .txt.
 * Updated: 2020-10-19
 *   Add reversible squares.
 * Updated: 2020-11-05
 *   Add odd-associative and even-associative.
 * Updated: 2020-11-16
 *   Check associative and pandiagonal for notMagic, otherMagic squares.
 * Updated: 2020-11-24
...
struct t_ways bent, parity, *Uzig, *Vzig, *VzigA;
...
enum magicType {
  ...
  Uzigzag, Vzigzag, VzigzagA, selfComplement,
  LS, DLS, AssociativeDLS, nearAssociativeDLS, PLS, WPLS, UltramagicDLS,
  Sym, DSym, CSym, nearCSym, nfr, nfc, nfr_nfc, nbd, selfTranspose, OLS, SOLS,
...
int typeCount[numTypes], *typeCountCompact, typeCountBent[5], typeCountParity[3];
...
bool isAssociative(int **x, const int nr, const int nc, int S2) {
//   -------------
...
bool isOddAssociative(int **x, const int nr, const int nc, const int S2) {
//   ----------------
  if (squareTypes[Associative].isType) return F; if ((S2&1)==1) return false;
...
bool isEvenAssociative(int **x, const int nr, const int nc, const int S2) {
//   -----------------
  if (squareTypes[Associative].isType) return F; if ((S2&1)==1) return false;
  const int nrm1=nr-1, ncm1=nc-1;
...
//   ------------
  return squareTypes[Ultramagic].isType=
    (squareTypes[normalMagic].isType|squareTypes[otherMagic].isType)&
...
//   ----------
  const int m=n-1; int Sk=0, S;
  for (int r=0; r<k; ++r) for (int c=0; c<k; ++c) Sk+=x[r][c]; S=Sk;
  
  for (int r=0; r<n; ++r) {
    if (r>0) {
      int i=r-1, l=r+k-1; if (l>=n) l-=n;
      for (int j=0; j<k; ++j) { S-=x[i][j]; S+=x[l][j]; } if (S!=Sk) return F;
    }
    for (int c=1; c<n; ++c) {
      for (int h=r; h<(r+k); ++h) {
        int i=(h>=n)?h-n:h, j=c-1, l=c+k-1; if (l>=n) l-=n; S-=x[i][j]; S+=x[i][l];
      }
...
bool isPandiagonal(int **x, const int n, int Sum) {
//   -------------
  if (((n&3)==2)&(squareType==normalMagic)) return isNearPandiagonal(x, n);
  if (Sum==0) for (int r=0; r<n; ++r) Sum+=x[r][r];
  ...
  if (panBack&panForward) {
    if (squareType==normalMagic) isComplete(x, n, Sum2);
...
//   --------------
  ...
  for (int r=0; r<nr; ++r) {
    for (int c=0; c<gc; ++c) if ((x[r][c]&1)!=(x[r][mc-c]&1)) { parLR=F; break; }
    if (!parLR) break;
  }
  if (parLR) { ++waysPar; parity.lrud|=left; }

  for (int r=0; r<gr; ++r) {
    for (int c=0; c<nc; ++c) if ((x[r][c]&1)!=(x[mr-r][c]&1)) { parUD=F; break; }
    if (!parUD) break;
  }
  if (parUD) { ++waysPar; parity.lrud|=up; }

  parity.ways=waysPar;
  return squareTypes[axialParity].isType=(waysPar>0);
} // hasAxialParity
...
bool hasTransposeParity(int **x, const int n) {
//   ------------------
  const int m=n-1;
...
//   ------------
  ...
  if (checkUnique(x, n)) {
    int S2=(chkSum+chkSum)/n;
    isAssociative(x, R, C, S2); isPandiagonal(x, n, chkSum); isUltramagic();
    isCompact(x,n); isComplete(x, n, S2); isMostPerfect(); isConcentric(x, n); isBiMagic(x, n);
...
//   --------------
  for (int i=0; i<=NN; ++i) numUsed[i]=F;
  for (int r=0; r<N; ++r) for (int c=0; c<N; ++c) {
    const int t=x[r][c]; if ((t>=0)&(t<=NN)) numUsed[t]=T;
  }
  for (int i=1; i<NN; ++i) if (!numUsed[i]) return F;
  if (numUsed[0]==numUsed[NN]) return F;
  return T;
} // allNumbersUsed

bool isReversible(int **x, const int n) {
//   ------------
...
//   -------------
  switch (i) {
    ...
    case VzigzagA:
      ...
      break;
    case axialParity:
      ++typeCountParity[parity.ways];
      if (parity.ways==2) ++typeCount[i]; else typeCountParity[0]=1; // there is 1 way
      break;
...
//   -----------
  ...
  *s++=' '; *s++='(';
  ...
  if (x.lrud&down) { if (started) { *s++=','; *s++=' '; }
    *s++='d'; *s++='o'; *s++='w'; *s++='n';
  }
  *s++=')'; *s='\0'; return (fprintf(wfp, "%s", waysBentStr)>0);
} // putWaysBent

bool putWayParity(struct t_ways x, FILE *wfp) {
//   ------------
  char waysStr[30], *s=waysStr; bool started=F;
  *s++=' '; *s++='1'; *s++='-'; *s++='w'; *s++='a'; *s++='y'; *s++=' '; *s++='(';
  if (x.lrud&left) { *s++='l'; *s++='e'; *s++='f'; *s++='t'; *s++='-';
                     *s++='r'; *s++='i'; *s++='g'; *s++='h'; *s++='t'; 
  } else { *s++='u'; *s++='p';  *s++='-';
           *s++='d'; *s++='o'; *s++='w'; *s++='n';
  }
  *s++=')'; *s='\0'; return (fprintf(wfp, "%s", waysStr)>0);
} // putWayParity
    ...
    if (squareTypes[i].isType) {
      ...
      switch (i) {
        ...
        case Sym:
          ...
          break;
       case axialParity:
         if (parity.ways!=2) { if (!putWayParity(parity, wfp)) return F; }
          break;
        default:
          break;
      }
...
//   -------------
  ...
  isPLS(x,R); isUltramagicDLS(x,R); hasAxialParity(x, R, R); hasSymmetricParity(x, R, R); hasTransposeParity(x, R);
...
//   ------------
  ...
  squareTypes[squareType].isType=T; parity.ways=0; parity.lrud=0;
  if (R==C) {
...
//   --------
  ...
  if ((squareType==LS)|(squareType==DLS)) return getLatinTypes(x, yRectangle, R, wfp);
    ...
    if (R==C) {
      if (R==1) {
        if (squareType==normalMagic) squareTypes[LS].isType=T;
      } else {
        if (R>=3) {
          if (R==3) {
            if (squareType==normalMagic) {
            ...
            if ((squareType==normalSemiMagic)|(squareType==normalMagic)) {
              ...
              if (squareType==normalMagic) {
                if (!isAssociative(x, R, R, Sum2)) {
                  if ((Sum2&1)==0)
                    isOddAssociative(x, R, R, Sum2)||isEvenAssociative(x, R, R, Sum2);
                }
                isPandiagonal(x, R, magicSum); isUltramagic(); isMostPerfect();
        ...
        if (squareType==notMagic) isReversible(x, R);
        if ((squareType==notMagic)|(squareType==otherMagic))
            { isAssociative(x, R, R, 0); isPandiagonal(x, R, 0); }
        hasAxialParity(x, R, R); hasSymmetricParity(x, R, R); hasTransposeParity(x, R);
      }
    } else { // non-square rectangle
      if (squareType==normalMagic) {
        if (!isAssociative(x, R, C, Sum2)) {
          if ((Sum2&1)==0) isOddAssociative(x, R, C, Sum2)||isEvenAssociative(x, R, C, Sum2);
        }
      }
      hasAxialParity(x, R, C); hasSymmetricParity(x, R, C);
    }
    return printType(wfp);
  }
} // getTypes
  ...
  for (int i=firstType; i<=lastType; ++i) {
     ...
     switch (i) {
      ...
      case VzigzagA:
        ...
        break;
      case axialParity:
        if (typeCountParity[1]!=0) printf("%10d %s 1-way\n", typeCountParity[1], squareTypes[i].name);
        break;
      default: break;
    }
  }
...
//   --------------
  ...
  for (int i=0; i<5; ++i) typeCountBent[i]=0; for (int i=0; i<3; ++i) typeCountParity[i]=0;
  for (int i=0; i<Nk; ++i) {
